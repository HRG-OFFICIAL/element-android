...
/**
 * Integration tests for AntiDebug SDK
 * Tests the complete integration of all security modules
 */
@RunWith(RobolectricTestRunner::class)
class AntiDebugIntegrationTest {
    ...
    fun testAntiDebugInitialization() {
        ...
        AntiDebug.init(realContext, false)
    ...
    @Test
    fun testAntiDebugInitializationWithContinuousMonitoring() {
        // Test initialization with continuous monitoring
        AntiDebug.init(realContext, true)
    ...
    fun testIndividualDetectionMethods() {
        ...
        val debuggerDetected = AntiDebug.isDebuggerAttached()
        val rootDetected = AntiDebug.isDeviceRooted()
        val emulatorDetected = AntiDebug.isRunningOnEmulator()
        val tamperDetected = AntiDebug.isApplicationTampered()
        val hooksDetected = AntiDebug.areHooksDetected()
        val suspiciousBehavior = AntiDebug.isSuspiciousBehavior()
    ...
    fun testImmediateSecurityCheck() {
        ...
        val result = AntiDebug.performImmediateSecurityCheck()
    ...
    fun testErrorHandling() {
        ...
        try {
            AntiDebug.init(mockContext, false)
        ...
        repeat(10) {
            val thread = Thread {
                try {
                    ...
                    AntiDebug.isDeviceRooted()
                    AntiDebug.performSecurityCheck()
                } catch (e: Exception) {
