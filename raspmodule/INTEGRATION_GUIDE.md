...
This guide provides comprehensive instructions for integrating the RASP SDK into Android applications. The SDK provides multiple layers of security protection including debugger detection, root detection, emulator detection, tamper detection, hook detection, behavioral analysis, and data protection.

## Quick Start

### 1. Add Dependency

Add the raspmodule to your app's `build.gradle`:
...
#### `RASP.init(context: Context, enableContinuousMonitoring: Boolean = false)`

Initializes the RASP SDK.

**Parameters:**
- `context`: Application context
- `enableContinuousMonitoring`: Whether to enable continuous background monitoring

#### `RASP.performSecurityCheck(): SecurityReport`
...
#### `RASP.getDataProtection(): DataProtection`
...
#### `RASP.getMonitoringStatistics(): MonitoringStatistics`

Gets current monitoring statistics.

**Returns:** `MonitoringStatistics` object with monitoring information

#### `RASP.performImmediateSecurityCheck(): SecurityCheckResult`
...
#### `RASP.startContinuousMonitoring()`
...
class SecureDataManager {
    private val dataProtection = RASP.getDataProtection()
...
Ensure your app's `build.gradle` includes the raspmodule dependency:
...
Initialize the RASP SDK as early as possible in your application lifecycle, preferably in the `Application.onCreate()` method.
...
```kotlin
RASP.init(context, enableContinuousMonitoring = false)
```
...
For support and questions about the RASP SDK, please refer to the documentation or contact the development team.
